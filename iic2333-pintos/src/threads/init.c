...
parse_options (char **argv)
{
	for (; *argv != NULL && **argv == '-'; argv++)
	{
		...
		char values[2];
		char* value1=strtok_r(value,",",&save_ptr);
		char* value2=strtok_r(NULL,"",&save_ptr);
	...
	/* Initialize the random number generator based on the system
     ...
     initial time to a predictable value, novalues[0]=strtok_r(value,",",&save_ptr);
		 st to the local time,
...
run_task (char **argv)
{
	...
	//max_turns_in_queue = atoi(argv[2]);
	//queue_number = atoi(argv[3]);
