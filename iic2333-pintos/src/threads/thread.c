...
int get_queue(struct thread* t){

	int qcompleted = get_age(t);
	int queue = qcompleted/A;
	if(queue>=N){
		return N-1;
	}
	return queue;
...
void da_yields(){
	...
	if ( !list_empty(&ready_list) )
	{
		...
		if (intr_context())
		{
			...
			if ( thread_current()->priority < t->priority ||(++thread_ticks >= (t->quantums_completed/A + 1)*TIME_SLICE) )
			{
...
int get_age(struct thread* t){


	return t->quantums_completed;
}
...
thread_print_stats (void)
{
	...
	printf("Tiempo promedio de espera en ticks %d\n", totalwaitTime/threadnum);
...
thread_block (void)
{
	...
	//thread_current() -> times_blocked+=1;
...
void heapSort(){
	...
	while(list_size(&ready_list)){
		...
		//printf("popping\n");
		struct list_elem* ele = list_pop_front (&ready_list);
		struct thread* t =  list_entry (ele, struct thread, elem);
		node nd;
		//	printf(t->name);
		//	printf(" queue: %d\n ", t->quantums_completed/A);

		int queue =(t->quantums_completed/A +1);
		if(queue>=N){
			queue = N;
		}
		nd.data=1000000 - queue *63 + t->priority;


		//printf("Priority del hs %d\n", nd.data);
		nd.elem=t;
		insertNode(&hp,&nd);
	...
	while(elem.data!=-1){
		...
		//printf("Priority: %d\n",elem.data);
		list_push_front(&ready_list, &elem.elem->elem);
		elem = getNextElement(&hp);
	}
...
thread_exit (void)
{
	...
	printf("   ----------------------------------\n");
	printf("Ticks de ejecucion: %d\n", curr->cpu);
	printf("Veces elegido: %d\n",curr->times_picked);
	printf("Veces que expiro su quantum: %d\n", curr->quantums_completed);
	printf("Cantidad de veces que estuvo esperando %d\n", curr->times_waiting);
	printf("Cantidad de veces que estuvo bloquedo %d\n", curr->times_blocked);


	int queue =(curr->quantums_completed/A +1);
	if(queue>=N){
		queue = N;
	}

	printf("Veces que cambio de cola %d\n", queue-1);
	totalwaitTime+= curr->times_waiting;
...
thread_yield (void)
{
	...
	if (cur != idle_thread){
		...
		//printf("Current quantums: %d \n", cur->quantums_completed);
		//printf("priority %d: \n",cur->priority);
...
void updateStatuses(){
			...
			e = list_next (e))
	{
		...
		if(t->status == THREAD_READY){
			...
			struct list_elem* ele = list_front (&ready_list);
			struct thread* next = list_entry(ele, struct thread, elem);
			if(t->tid != next->tid){
				t->times_waiting+=1;
			}
		}
...
thread_set_priority (int new_priority)
{
	...
	heapSort();
	struct list_elem* ele = list_front(&ready_list);
	struct thread* ret = list_entry (ele, struct thread, elem);


	if (thread_current()->priority< ret->priority)
	{
		da_yields();
	}
	intr_set_level (old_level);
...
pick_scheduler(void)
{
	...
	case SCH_FCFS:
	default:
		printf("Using N-QUEUES as the CPU Scheduler\n");
		printf("Using Age A=%d Queues=%d \n",A,N);
